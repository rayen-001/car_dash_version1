 ...
 * (see Phase 16.4 spec / paper reference). Key principles:
 *
 *  • Strict A4 portrait (210 × 297 mm) bounded layout via @media print.
 *  • Asymmetrical 12-col grid: col-span-7 (~55%) left + col-span-5 (~45%) right.
 *  • Bilingual (FR / AR) labels via <BiLabel>, RTL-aware.
 *  • Settings-driven header (FR-left / Logo-center / AR-right) — ZERO hardcoded
 *    agency strings. Empty settings render as dotted-placeholder rows.
 *  • Serial number derived stable from booking.id (no schema change required).
 *  • 3-tier fuel telemetry: pickup gauge, return gauge, damage-tracker frame.
 *  • 9-row financial ledger (paper-form parity).
 *  • Dual print mode: color-on (default, matches paper) vs monochrome toggle.
 *
 * Multi-tenant isolation: no DB writes from this component. Receives `booking`
 * + `businessSettings` as props (both already owner-scoped upstream by the
 * caller's getBusinessSettings() and the bookings page SELECT).
 */
...
/** Stable 6-char serial derived from booking ID (matches "N° 009175" paper format).
 *  If a real serial_number column gets added later, swap to (booking.serial_number ?? derived). */
function serialFromId(id: string): string {
  const hex = id.replace(/-/g, '').toUpperCase()
  // Take the last 6 hex chars and present as a 6-digit-style serial.
  return hex.slice(-6)
}
...
export default function BookingAgreementModal({
  ...
  /** Render either the provided value OR a dotted-placeholder slot for handwriting. */
  ...
  return (
    <div className="modal-overlay print-agreement-container" style={{ background: 'rgba(15, 15, 15, 0.85)' }}>
      ...
      >
        ...
        >
          <span style={{ fontSize: '0.85rem', color: '#475569', fontWeight: 500 }}>
            Contrat de Location — A4 Print Preview
          </span>
        ...
        >
          ...
          {/* ── FOOTER BANNER (full-bleed black) ── */}
          <div
      ...
      {/* ── Print CSS — locked A4 portrait, color preserved by default, mono toggle ── */}
      <style
