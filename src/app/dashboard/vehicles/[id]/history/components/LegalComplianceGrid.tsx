...
export default function LegalComplianceGrid({ legalDocs, vehicleId, onEditDoc }: LegalComplianceGridProps) {
  ...
  const handleConfirmRenewal = async (type: 'assurance' | 'visite_technique' | 'laissez_passer') => {
    ...
    try {
      ...
      showToast(`Document successfully renewed for ${chosenDuration === '6m' ? '6 Months' : '1 Year'}!`, 'success')
      router.refresh()
    } catch (err: any) {
  ...
  const getDocStatus = (docType: string) => {
    ...
    const doc = tempDoc 
      ? { doc_type: docType, expiry_date: tempDoc.expiry_date, notes: 'Quick Auto-Renewed' } 
      : legalDocs.find((d) => d.doc_type === docType)

    if (!doc) return { label: 'Missing', class: 'missing', cardClass: 'missing-card', dateStr: 'Not Logged' }
    ...
    if (daysDiff < 0) {
      return { 
        ...
        dateStr: expiryDate.toLocaleDateString(),
        notes: doc.notes
      }
    } else if (daysDiff <= 30) {
      return { 
        label: `Expiring in ${daysDiff} days`, 
        class: 'expiring', 
      ...
      return { 
        label: `🟢 Compliant / Active (${daysDiff} days left)`, 
        class: 'active', 
  ...
  const docTypes: { type: 'assurance' | 'visite_technique' | 'laissez_passer'; title: string }[] = [
    { type: 'assurance', title: 'Insurance (Assurance)' },
    { type: 'visite_technique', title: 'Technical Inspection' },
    { type: 'laissez_passer', title: 'Transport Authorization' }
  ]
          ...
          >
            <div>
              ...
              <h3 className={styles['legal-doc-name']}>
                {type === 'assurance' ? 'Assurance Policy' : type === 'visite_technique' ? 'Visite Technique' : 'Laissez-Passer'}
              </h3>

              <div className={styles['legal-expiry']}>
                Expiry Date: <strong>{status.dateStr}</strong>
              </div>
            ...
            <div>
              {(status.label === 'Missing' || status.label === 'Expired') && (
                <div 
                  style={{ 
                    marginTop: '1rem', 
                    display: 'flex', 
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
                    <span style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.6)' }}>Renewal Coverage:</span>
                    <select
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (loadingType !== type) {
                        e.currentTarget.style.background = 'rgba(229, 193, 125, 0.25)'
                        e.currentTarget.style.boxShadow = '0 0 8px rgba(229, 193, 125, 0.15)'
                      }
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background = 'linear-gradient(90deg, rgba(229, 193, 125, 0.08), rgba(229, 193, 125, 0.18))'
                      e.currentTarget.style.boxShadow = 'none'
                    }}
                  >
                    {loadingType === type ? 'Saving...' : '⚡ Confirm Renewal'}
                  </button>
                </div>
              )}
